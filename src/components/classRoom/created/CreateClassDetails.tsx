...
function ClassDetails({ classCode }: { classCode: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/class/getClassData/${classCode}`);
  ...
  return (
    <div className="flex flex-col h-32 w-full">
      <div className="relative h-32 md:h-120 bg-orange-100">
        <h1 className="absolute left-1/4 top-1/4 transform -translate-y-1/3 font-bold text-gray-700 md:text-xl">
        ...
        <h1 className="absolute left-1/4 top-3/4 transform -translate-y-1/2 text-gray-700 md:text-md">
