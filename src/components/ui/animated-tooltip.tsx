  ...
  return (
    <>
      <SidebarContext.Provider value={{ side, setSide }}>
        ...
        {items.map((item, idx) => (
          <div
            className="-mr-6 relative group hidden sm:block" // Hide on small screens
            key={item.name}
          ...
          >
            ...
            <Image
              ...
              src={item.image}
              alt={item.name}
              className="object-cover !m-0 !p-0 object-top rounded-full h-9 w-11 border-2 group-hover:scale-105 group-hover:z-30 border-white relative transition duration-500"
            />
          </div>
        ))}
